...
BeforeAll {
  ...
  # Import ConfigurationManager module (only if not already loaded)
  ...
  $moduleLoaded = Get-Module -Name 'ConfigurationManager' -ErrorAction SilentlyContinue
  if (-not $moduleLoaded) {
    Import-Module -Name $modulePath -Force -ErrorAction Stop
  
  # Initialize performance mocks to prevent slow console I/O
  Initialize-PerformanceMocks -ModuleName 'ConfigurationManager'
  }
}
